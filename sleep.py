...
first_meal = st.sidebar.time_input("First meal after waking", value=datetime.strptime("08:00", "%H:%M").time())
...
# Base Layer: Time in Bed (Lighter color, wider bar)
...
# Top Layer: Actual Time Slept (Darker color, slightly narrower so base is visible)
fig.add_trace(go.Scatter(
...
fig.add_vline(
    x=dt_last_meal.timestamp() * 1000, # Plotly expects milliseconds for vertical lines on datetime axes
...
# Vertical Line: First Meal
fig.add_vline(
    x=dt_first_meal.timestamp() * 1000, 
    line_dash="dot", line_color="green", annotation_text="First Meal", 
...
fig.update_layout(
    ...
    showlegend=True,
    xaxis=dict(
        tickformat="%I:%M %p", # Format as 12-hour AM/PM
